...
На самом деле для цикла `for` существует интересная особенность: если нам нет необходимости использовать перебираемые элементы, то мы можем указать символ `_` вместо переменной цикла:
...
Этот способ используется на практике гораздо чаще, чем обычный `open` и `close`. Почему эта «магия» так работает, расскажу дальше по курсу.
...
3. **Копирование файла**: Создайте текстовый файл `source.txt`. Написать программу, которая читает содержимое `source.txt` и записывает его в новый файл `destination.txt`.

4. **Анализ файла**: Создайте текстовый файл со списком чисел (каждое число на новой строке). Написать программу, которая читает файл и печатает самое большое число.

5. **Конвертация файлов**: Создайте текстовый файл с температурами в градусах Цельсия (каждая на новой строке). Написать программу, которая читает файл, конвертирует температуры в градусы Фаренгейта и записывает результаты в новый файл.

6. **Подсчёт слов в файле**: Написать программу, которая считает количество слов в текстовом файле.
