...
2) Есть два списка одинаковой длины. Создать словарь, где ключи — это элементы первого списка, а значения — элементы второго. Например: `[1, 2, 3]`, `[4, 5, 6]`, результат `{1: 4, 2: 5, 3: 6}`.
...
Существует ещё множество разных модулей — вы будете знакомиться с ними по мере необходимости. Вряд ли есть человек, который знает все стандартные модули, а тем более те, которые можно установить дополнительно.
