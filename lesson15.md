...
Всё очень просто. Чтобы удалить запись, используется вот такой синтаксис:
...
Обратите внимание: поле `employee_id` содержит модификатор `UNIQUE` — именно он обеспечивает нам связь «один к одному». Без него
это была бы связь «один ко многим», как в следующем примере.
