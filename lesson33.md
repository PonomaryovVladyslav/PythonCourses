# Урок 33. TemplateView, RedirectView, CreateView, UpdateView, DetailView, ListView, ModelForm
...
ModelForm - это тип формы который генерируется напрямую из модели. Используется для изменения поведения действий с
моделями. Можно переопределить определенные действия с такими формами.
...
Метод `save()` может принимать аргумент `commit`, по умолчанию `True`.

Если указать коммит как фолс, объект не будет сохранён в базу, будет только создан предварительный объект для создания,
