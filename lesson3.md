...
Так как None не является привычным типом данных нельзя проверить объект на `None` через `==`, это делается так:
...
> Шаг при необходимости может быть отрицательным!
...
Python - это язык программирования, который предоставляет множество встроенных (**built-in**) методов для работы с
