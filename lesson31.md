...
<details>
  ...
  - [Лекция 34. Linux. Всё, что нужно знать для деплоя](lesson34.md)
...
серверу может продолжать работу, даже если ответ на запрос ещё не пришёл. Асинхронное взаимодействие позволяет получить
...
Celery — это простая, гибкая и надёжная распределённая система для обработки огромного количества сообщений, включая в
...
протокола AMQP, описанным в спецификации. Отправитель передаёт сообщение брокеру, а тот доставляет его получателю.
RabbitMQ реализует и дополняет протокол AMQP.

#### Redis

**Redis** (Remote Dictionary Server) — это быстрое хранилище данных типа «ключ — значение» в памяти с
открытым исходным кодом для использования в качестве базы данных, кеша, брокера сообщений или очереди.
...
> **Celery 4.0+ официально не поддерживает Windows.** Для разработки на Windows рекомендуется использовать WSL 2 или Docker.

Варианты запуска на Windows:
- **WSL 2** (рекомендуется) — полноценный Linux внутри Windows
- **Docker** — контейнеризация решает все проблемы совместимости
...
Результат будет иметь достаточно большое количество методов и атрибутов.

Основные два метода это `ready()` и `get()`:

`ready()` — булево поле, которое отвечает за то, завершилась задача или ещё в процессе.

`get()` — ждёт выполнения задачи и возвращает результат. Рекомендуется использовать после `ready()`, чтобы не ждать
выполнения впустую.
...
Документация: https://docs.celeryq.dev/en/stable/userguide/canvas.html
...
1. `countdown` — выполнить через определённый промежуток времени
...
4. `link` — выполнить другую задачу по завершении текущей, основываясь на результатах текущей
...
`app.autodiscover_tasks()` — эта строчка будет отвечать за автоматический поиск тасков во
всех приложениях.
...
Когда несколько потоков пытаются одновременно изменить общий объект, возможны конфликты. Представьте, что 5 человек
...
### Python 3.13+ и Free-threaded Python (PEP 703)

В октябре 2024 года вышел Python 3.13, который принёс **экспериментальную** поддержку режима без GIL (free-threaded Python). Это результат многолетней работы над PEP 703.

**Что изменилось:**

1. **Опциональный режим без GIL**: Python 3.13 можно собрать с флагом `--disable-gil`. В этом режиме GIL полностью отключён, и потоки могут выполняться по-настоящему параллельно.

2. **Экспериментальный статус**: Режим без GIL пока экспериментальный. Не все библиотеки (особенно C-расширения) готовы к работе без GIL. Ожидается, что полная поддержка появится в Python 3.14–3.15.

3. **Как попробовать**: Нужно установить специальную сборку Python (например, `python3.13t` — "t" означает "free-threaded") или собрать из исходников с `--disable-gil`.

**Что это значит для разработчиков:**

- **Пока рано переходить**: Большинство проектов должны продолжать использовать стандартный Python с GIL
- **Следите за библиотеками**: Популярные библиотеки (NumPy, pandas и др.) постепенно добавляют поддержку free-threaded режима
- **Тестируйте заранее**: Если ваш проект активно использует многопоточность, можно начать тестирование на free-threaded Python

**Важно:** Даже с отключённым GIL вам всё ещё нужны механизмы синхронизации (`Lock`, `Queue` и т.д.) для защиты общих данных от race conditions!

### Python 3.14: Как работает Python без GIL

Python 3.14 продолжает развитие free-threaded режима. Давайте разберём, как именно Python работает без GIL — это поможет понять, почему переход занимает так много времени.

#### Проблема атомарных операций

Раньше GIL гарантировал, что только один поток выполняет Python-код. Это позволяло использовать простые операции для подсчёта ссылок:

```c
// С GIL — просто и быстро (1 цикл CPU)
obj->refcount++
```

Без GIL нужны **атомарные операции** на уровне CPU:

```c
// Без GIL — атомарная операция (10-50 циклов CPU)
atomic_add(&obj->refcount, 1)
```

Атомарные операции в 10-50 раз медленнее! Как это решить?

#### Biased Reference Counting (смещённый подсчёт ссылок)

Каждый объект теперь имеет **два счётчика ссылок**:

| Счётчик             | Тип                  | Кто обновляет          |
|---------------------|----------------------|------------------------|
| **Local refcount**  | Быстрый, неатомарный | Поток-владелец объекта |
| **Shared refcount** | Атомарный            | Другие потоки          |

**Почему это работает:** Большинство объектов используются только одним потоком. Поэтому 90%+ операций `INCREF`/`DECREF` — быстрые и неатомарные.

#### Многоуровневая модель синхронизации

Без GIL Python использует три уровня защиты:

**1. Per-Object Mutex (PyMutex)**
- Каждый `PyObject` имеет лёгкий 1-байтовый мьютекс
- Защищает внутренние операции (изменение размера списка, обновление словаря)

**2. Container-Level Locking**
- Операции над `dict`, `list`, `set` захватывают собственные блокировки
- Гарантирует атомарность многошаговых операций (`dict.setdefault`, `list.append`)

**3. Consistent Lock Ordering**
- Каждому типу блокировки присвоен глобальный ранг
- Все операции захватывают блокировки в строгом порядке рангов
- **Гарантирует отсутствие deadlock'ов**

#### Thread-Safe Memory Management

**1. Аллокатор mimalloc**
- Каждый поток имеет свою арену памяти
- Отложенная очередь для освобождения памяти между потоками
- Потоки выделяют память независимо друг от друга

**2. Per-thread free-lists**
- Списки свободных объектов (tuples, frames, dict entries) перенесены в `PyThreadState`
- Нет глобальных пулов объектов — нет блокировок, нет гонок!

#### Safe Points и Stop-the-World GC

**Safe Points (точки синхронизации)**
- Потоки приостанавливаются только на определённых границах байткода
- Используются для: обновления типов, изменения состояния импорта, инвалидации кэшей, запуска GC
- Заменяют неявную глобальную синхронизацию, которую раньше обеспечивал GIL

**Stop-the-World Garbage Collector**
- Все потоки переходят в Safe Points и приостанавливаются
- GC получает эксклюзивный контроль над интерпретатором
- Выполняется: обнаружение циклов, согласование локальных и shared refcount'ов
- STW-паузы редки и обычно короче **1 мс**

#### Итог: почему это сложно

Удаление GIL — это не просто "убрать одну блокировку". Это полная переработка:
- Системы подсчёта ссылок
- Аллокатора памяти
- Всех встроенных контейнеров
- Сборщика мусора
- Механизма синхронизации потоков

Именно поэтому переход занимает несколько версий Python.

> `3.14t` — больше не эксперимент, но и не поведение по умолчанию. Это опция, которую можно выбрать, используя альтернативный интерпретатор.

## Multiprocessing (Многопроцесность)
...
В Python обычные структуры данных (`list`, `dict`) **не являются потокобезопасными**, если несколько потоков
