...
- Операции вставки/удаления аналогичны обычным спискам, но важно корректно перенастраивать ссылки, чтобы сохранять целостность кольца.
...
Graph) - направленный ациклический граф. Потому что на нем работает GIT, о чем мы и поговорим на следующем занятии.
...
Python использует в качестве базового алгоритма сортировки Timsort, который является гибридным алгоритмом, сочетающим
в себя сортировку слиянием и вставками. Timsort был разработан Тим Петерсом и впервые использовался в Python 2.3.
Детали реализации нам не очень важны, но в реальности он работает немного быстрее чем алгоритмы которые мы обсудили.
...
12. По отсортированному списку постройте почти сбалансированный BST (build_bst). Сравните среднюю глубину поиска со
    случайным порядком вставок.
13. Найдите все пути от корня до листьев (root-to-leaf paths) в дереве из словаря смежности.
