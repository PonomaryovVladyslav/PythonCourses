...
`--locale LOCALE, -l LOCALE` нужно, чтобы указать на какой язык планируется перевод (на самом деле повлияет только на то,
...
1. Снять фекстуру с вашей базы, изучить полученный файл.
2. Для вашего модуля создать дата миграцию, которая будет создавать два новых товаров.
3. Написать менедж команду, что бы отклонить все заявки на возврат.