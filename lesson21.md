...
> В рамках обучения вы будете запускать сервер прямо на ваших компьютерах, при разработке так обычно и делается.
...
Что произойдет в этой ситуации. Вы сообщили консоли, что ходите запустить `python` из виртуального окружения, через него
...
> Эта команда запускает ваш `сервер`. И если вы хотите, что бы ваш сайт работал, сервер, должен быть запущен!
...
`127.0.0.1` можно заменить на `0.0.0.0` или слово `localhost`, все три варианта практически взаимозаменяемы.
...
Она сообщает всей системе, что в качестве настроек нужно использовать файл `setting.py` из папки `mysite`
...
Мы создали функцию, которая принимает один параметр `request` типа `HttpResponse` и возвращает объект
типа `HttpResponse`.
