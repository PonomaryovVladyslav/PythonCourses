...
Для того, что бы обрабатывать шаблоны мы дожны "рассказать" Django где именно искать эти самые шаблоны, для этого нужно открыть `mysite/settings.py` и отредактировать его.

Сеттингс большой файл сгененрированый при старте проекта, что в нём лежит и зачем нам это нужно будем разбираться по ходу обучения.
...
Значения переданны, но пока они не как не используются, давай те же посмотрим, как отобразить переменные в шаблоне!
...
Тег url позволяет нам сгенерировать урл по его имени. это очень удобно если адресс меняется, а его имя нет.
...
Наследование шаблонов нужно, что бы не писать одно и тоже отображние много раз, как часто вы видели сайты где сверху, снизу, слева, справа итд. всегда одно и тоже, а меняется только "середина", самый простой способ так сделать это наследование.

Тут нам и помогут наши волжебные теги!

Создадим в папке `templates` новые файлы `base.html` и `first.html`

и изменим файлы `templates/index.html` и функцию `first` в `myapp/views.py`
...
Например `date`, `default`, `join`, `capfirst` на самом деле их огромное кол-во весь список встроенных фильтров можно посмотреть [Тыц](https://docs.djangoproject.com/en/2.2/ref/templates/builtins/#add)

## Кастомные темплейт теги и фильтры

На самом деле, стандартным набором дело не ограничивается и если нужно можно дописать свои теги и фильтры, почитать об этом можно вот [тут](https://docs.djangoproject.com/en/3.1/howto/custom-template-tags/)
...
Для статических урлов сделать наследующиеся от базового, но с разным текстом (можно и оформлением)

Для динамических, выводить текст о том чётный введен или нечётный (логика в темплейтах), есди введён слаг, выводить его при помощи include в добавочной html (добавленной из отдельного файла).
