...
При загрузке проекта в реальное использование переменную `DEBUG` нужно установить в `False` и обрабатывать статические файлы внешними средствами, поговорим о них ниже.
...
**Про медиа файлы в urls.py:**

В локальной разработке Django может обслуживать медиа файлы через встроенный сервер. Для этого в `urls.py` добавляют:
...
**Важно:** Этот код работает **только при `DEBUG=True`**. В продакшене (`DEBUG=False`) медиа файлы обслуживает nginx напрямую — это быстрее и безопаснее. Мы настроим это ниже в разделе про nginx.
