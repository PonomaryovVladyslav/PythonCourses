...
`action` описывает, куда форма по результату должна обращаться, в нашем
случае это будет `url`. 

`method` - отвечает за метод отправки, варианты это `get` и `post`. `get` будет использован по
умолчанию если не указан явно.
...
Тег добавил нам специальное скрытое поле, со сгенерированным кодом. Почему он будет автоматически обработан мы разберём в
следующих занятиях.
